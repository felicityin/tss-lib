...
import (
	...
	"github.com/bnb-chain/tss-lib/v2/crypto/logproof"
...
func (round *round2) Start() *tss.Error {
	...
	for j := range Ps {
		...
		if err := encProof.Verify(ProofParameter, contextJ, round.temp.kCiphertexts[j],
			round.key.PaillierPKs[j].N, round.key.RingPedersenPKs[i],
		); err != nil {
	...
	G, err := crypto.NewECPoint(round.EC(), round.EC().Params().Gx, round.EC().Params().Gy)
	if err != nil {
	...
	for j, Pj := range round.Parties().IDs() {
		...
		logProof, err := logproof.NewKnowExponentAndPaillierEncryption(ProofParameter, contextI, round.temp.k,
			round.temp.rho, round.temp.kCiphertexts[i], round.key.PaillierPKs[i].N, round.key.RingPedersenPKs[j], Ri, G)
		if err != nil {
